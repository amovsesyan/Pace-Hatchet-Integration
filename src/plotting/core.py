...
# from __editable___hatchet_1_4_0_finder import GraphFrame
...
def timeline(trace_events, start_time, end_time, show_depth=False, instant_events=False):
    ...
    def update_depths(attr, old, new):
        event = Event()
    ...
    depth_slider = Slider(start=0, end=events['_depth'].max(), value=0, step=1, title="Depth")
    depth_slider.on_change('value', update_depths)
    # depth_slider.on_event()
    ...
    def update_cds(event):
        ...
        in_bounds = events[
            (
                (events["start_time"] < x1) & (events["end_time"] > x0) & (events['_depth'] <= depth_slider.value)
            )
        ].copy(deep=False)
...
def profile_trace_view(gf: GraphFrame):
    ...
    def traverse(node, parent_index = -1):
        ...
        parent_start_time = last_end_times[parent_index]
        start_time = parent_start_time + offsets[parent_index]
        ...
        depths.append(depth)
        last_end_times.append(start_time)
        offsets.append(calc_offsets(node))
        last_end_times[parent_index] = end_time
    ...
    for root_node in gf.graph.roots:
        ...
        start_times.append(start_time)
        end_times.append(end_time)
        offsets.append(calc_offsets(root_node))
        depths.append(0)
        last_end_times.append(start_time)
